...
map<uint8_t, float> Genome::compute(const map<uint8_t, uint16_t> &stimuli) {
    ...
    for (const auto &item: stimuli) {
        if (inputs.find(item.first) != inputs.end()) {
            inputs[item.first] = (float) item.second / (float) ((1 << 16));
        }
    }
    ...
    for (auto &item: neurons) {
        item.second = sigmoid(item.second);
    }
...
std::array<std::array<double, 3>, 256> data = {{
...
void setColor(double value) {
    ...
    value = 1 - value;
    int index = value * (data.size() - 1);
    double fraction = value * (data.size() - 1) - index;

    double r = data[index][0] * (1 - fraction) + data[index + 1][0] * fraction;
    double g = data[index][1] * (1 - fraction) + data[index + 1][1] * fraction;
    double b = data[index][2] * (1 - fraction) + data[index + 1][2] * fraction;

    glColor4d(r, g, b, 1.0);
}

void Genome::render(GFX *g, double dx, double dy, double min, double max) {

    glPushMatrix();
    setColor((distance - min) / (max - min));
...
void Genome::drawChar(GFX *g, double min, double max) {
    glPushMatrix();
    setColor((distance - min) / (max - min));
...
vector<Gene> Genome::mingle(Genome *mate, int mutationRate) {
    ...
    for (int i = 0; i < core.size(); ++i) {
        ...
        if (randomUInt() % mutationRate == 1) {
            flipRandomBit(insert);
        }
